...
public function journaldebord() {
    ...
    if ($this->estUtilisateurConnecte()) {
        ...
        $utilisateurConnecte = $_SESSION['oUtilConn'];
        $utilisateur_id = $utilisateurConnecte->getUtilisateur_id();
        $nomUtilisateur = $utilisateurConnecte->getUtilisateur_nom();
    } else {
    ...
    // Récupérer les données du journal depuis le modèle
    $journalEntries = $this->oRequetesSQL->getJournalUtilisateur($utilisateur_id);

    // Utiliser Twig pour afficher les données
    (new Vue)->generer(
        "afficherJournal", 
        [
            'oUtilConn' => $utilisateurConnecte,
 ...
 */
private function estUtilisateurConnecte() {
    return isset($_SESSION['oUtilConn']) && $_SESSION['oUtilConn'] instanceof Utilisateur;
}
